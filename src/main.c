...
int main(int argc, char *argv[]) {
    ...
    if (udp_socket < 0) exit_err_status("FAILED TO CREATE UDP SOCKET:");
    ...
    while (!to_exit) {
        ...
        if (!cmd_list->is_cluster) {
            ...
            for (size_t i = 0; i < cmd_list_size; i++) {
                ...
                if (!is_cluster)
                    ...
                    if (command->stdout_fd != 0) {
                        ...
                        recv_command(udp_socket, INADDR_ANY,
                                     ntohs(self_addr.sin_port));
                        for (int i = 0; i < global_cluster_data.addr_count;
                             i++) {
                            PRINTF_FG_CYAN("-------OUTPUT OF NODE n%d-------",
                                           i);
                            PRINTF_FG_WHITE("\n");
                            recv_output(udp_socket,
                                        global_cluster_data.ip_address_array[i],
                                        global_cluster_data.ports_array[i]);
                        }
                    }
            ...
            if (cmd_list->is_trip) {
                ...
                while ((bytes_read =
                            read(cmd_list->ofd, buffer, PIPE_BUF_SIZE)) > 0) {
                    for (int i = 0; i < 3; i++)
