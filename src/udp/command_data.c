...
void send_command(char *command, int udp_socket, in_addr_t dest_ip, int port) {
    ...
    for (size_t i = 0; i < cmd_len; i += DEFAULT_UDP_SIZE) {
        size_t chunk_size =
            (cmd_len - i > DEFAULT_UDP_SIZE) ? DEFAULT_UDP_SIZE : cmd_len - i;
...
void recv_command(int udp_socket, in_addr_t src_ip, int port) {
    ...
    while (1) {
        ...
        udp_buffer =
            realloc(udp_buffer, sizeof(char) * (buf_size + DEFAULT_UDP_SIZE));
        ptr += DEFAULT_UDP_SIZE;
        buf_size += DEFAULT_UDP_SIZE;
    }

    int fds[2];
    if (pipe(fds) != 0) exit_err_status("PIPE FAILED WITH: ");
    ...
    cmd_list->pipefds = realloc(
        cmd_list->pipefds, sizeof(*cmd_list->pipefds) * cmd_list->pipe_count);
    ...
    for (int i = 0; i < cmd_list->pipe_count; i++)
        if (cmd_list->pipefds[i] != fds[0]) close(cmd_list->pipefds[i]);

    while ((bytes_read = read(fds[0], buf, PIPE_BUF_SIZE)) > 0) {
        if (sendto(udp_socket, buf, bytes_read, 0, (struct sockaddr *)&src_addr,
                   sizeof(src_addr)) < 0)
            exit_err_status("SENDTO FAILED WITH: ");
        bzero(buf, PIPE_BUF_SIZE);
    }
    ...
    for (size_t i = 0; i < cmd_list_size; i++) {
        ...
        for (size_t i = 0; i < cmd->size - 1; i++) free(cmd->cmd_arr[i]);
        free(cmd->cmd_arr);
    }
